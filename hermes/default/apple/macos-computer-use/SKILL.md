...
### DaVinci Resolve Node Graph (Known Limitation)
The Resolve node graph is a **custom Canvas view** that does not fully expose macOS Accessibility (AX) elements. The following **will not work reliably** via `computer_use`:
- `Option+S` / `Alt+S` to add serial nodes
- Arrow key navigation between nodes
- Right-click context menus on nodes
- Menu bar access (`Node > Add Serial Node`)
- SOM element detection on individual nodes

**Workarounds:**
1. **Manual workflow** — Provide click-by-click instructions with keyboard shortcuts for user execution
2. **PowerGrade `.drx` templates** — Build once, drag & drop onto clips
3. **MCP Integration** — Use `mcp_davinci_resolve_timeline_item_color` with `safe_apply_drx` or `propose_grade`
4. **AppleScript for top-level only** — Window focus, menu bar, but not node graph internals

**Diagnostic:** If `capture(mode="som", app="DaVinci Resolve")` shows no numbered overlays on the node graph area, the Canvas is not AX-accessible.

## When NOT to use `computer_use`\n\n- Web automation you can do via `browser_*` tools — those use a real\n  headless Chromium and are more reliable than driving the user's GUI\n  browser. Reach for `computer_use` specifically when the task needs the\n  user's actual Mac apps (native Mail, Messages, Finder, Figma, Logic,\n  games, anything non-web).\n\n## Note: macOS Mail.app automation is supported. Use `computer_use` with `app="Mail"` to interact with your unified inbox across Gmail, Outlook, and iCloud accounts. See references/apple-mail-automation.md for common patterns.
