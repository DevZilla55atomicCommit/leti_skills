...
Usage:
    META_API_KEY="your-key" PYTHONPATH="/path/to/site-packages" python3 meta_adapter.py
...
def convert_messages_to_input(messages: list[ChatMessage]) -> list[dict]:
    ...
    for msg in messages:
        content = msg.content
        # Meta Responses API uses different content types per role:
        # user -> input_text, assistant -> output_text, system -> input_text
        ...
        if isinstance(content, str):
            ...
            for part in content:
                if isinstance(part, dict) and "type" in part:
                    ...
                    new_content.append({"type": content_type, "text": part})
                else:
                    new_content.append({"type": content_type, "text": str(part)})
            content = new_content
...
async def chat_completions(req: ChatRequest):
    ...
    try:
        ...
        # Default to 2000 tokens to accommodate Muse Spark's heavy reasoning
