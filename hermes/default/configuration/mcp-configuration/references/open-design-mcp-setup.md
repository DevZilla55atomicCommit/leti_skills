# Open Design MCP Server Setup for Hermes Agent

## Overview
Open Design (OD) is a local-first design workspace that runs as a packaged macOS app. It exposes an MCP server that Hermes can consume to generate design artifacts (prototypes, decks, dashboards, images, video, HyperFrames).
...
# Or use the install script (requires PATH fix for /usr/bin/od collision):
