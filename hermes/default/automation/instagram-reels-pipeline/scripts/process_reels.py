...
def check_cookie_freshness(cookies_file):
    """Verify sessionid cookie hasn't expired — FAIL if < 1 hour remaining"""
    try:
            ...
            for line in f:
                ...
                if len(parts) >= 7 and parts[5] == 'sessionid':
                    ...
                    if remaining_hours <= 0:
                        print(f"❌ Cookies EXPIRED (sessionid expired {abs(remaining_hours):.1f}h ago)")
                        return False
                    elif remaining_hours <= 1:
                        print(f"❌ Cookies expire within 1 hour ({remaining_hours:.1f}h remaining) — REFRESH REQUIRED")
                        return False
                    else:
                        print(f"✅ Cookies valid ({remaining_hours:.1f}h remaining)")
                        return True
    except Exception as e:
        print(f"⚠️  Could not check cookie freshness: {e}")
        print("   Assuming valid — but verify manually if failures occur")
    return True  # Assume ok if check fails, but warn
...
def main():
    ...
    # Pre-flight cookie check — MANDATORY
    print("\n🔍 Pre-flight: Cookie freshness check...")
    if not check_cookie_freshness(COOKIES_FILE):
        print("\n❌ ABORTING: Cookies invalid or expiring within 1 hour.")
        print("   Refresh: Login to Instagram → Get cookies.txt LOCALLY extension →")
        print("   Save as ~/Downloads/cookies_www.instagram.com_YYYY-MM-DD.txt")
        print("   Update COOKIES_FILE in script if path differs.")
        sys.exit(1)
    ...
    # Load reel URLs
    if not check_cookie_freshness(COOKIES_FILE):
        print("❌ Refusing to run with expired cookies. Refresh and retry.")
        sys.exit(1)
