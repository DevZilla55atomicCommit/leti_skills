#!/usr/bin/env python3
"""
Auto-Processor — Fully automated 6-stage pipeline for Instagram Reels
Input: RTF file with URLs | Output: Vision reports, Hermes skills, Vault notes, GIFs
Runs in background with notify_on_complete
"""
...
async def download_batch(urls: List[Tuple[str, str]], batch_num: int) -> List[Dict]:
...
async def vision_analyze_frame(image_path: str, reel_id: str, timestamp: float) -> Dict:
    ...
    # Use the vision_analyze tool via subprocess
    # For now, create placeholder - in real use this calls the vision tool
    return {
...
async def analyze_reel_vision(reel_id: str, extraction: Dict) -> Dict:
    ...
    for fp, ts in zip(frame_paths, timestamps):
        detail = await vision_analyze_frame(fp, reel_id, ts)
...
description: |
  ...
  Educational focus: {summary.get('educational_value', '')}
version: 1.0.0
category: creative
tags:
  - davinci-resolve
...
### Composition & Framing
{chr(10).join(f"- **{t}**" for t in summary.get('techniques', []) if any(kw in t.lower() for kw in ['framing', 'composition', 'foreground', 'depth', 'leading']))}

### Camera & Lens
{chr(10).join(f"- **{t}**" for t in summary.get('techniques', []) if any(kw in t.lower() for kw in ['depth', 'macro', 'focus', 'bokeh', 'camera', 'lens', 'pov', 'angle', 'wide']))}

### Lighting
{chr(10).join(f"- **{t}**" for t in summary.get('techniques', []) if any(kw in t.lower() for kw in ['light', 'golden', 'natural', 'rim', 'backlight']))}
...
### Practice Exercises
1. **Golden Hour Grade**: Import footage → Build 5-node structure → Match warm/cool split → Add film grain
2. **Framing Within Frame**: Find architectural/natural frames → Power Window to isolate → Blur/darken outside, sharpen inside → Animate if frame moves
3. **Foreground Depth**: In Fusion, add leaf/branch elements → Track to camera → Blend Soft Light, animate subtle parallax
...
- **Educational Value**: {summary.get('educational_value', '')}
"""
...
def cleanup_reel(reel_id: str):
    for dir_path in [MP4_DIR, FRAMES_DIR, GIFS_DIR]:
...
async def main():
    ...
    # Setup directories
    for d in [TEMP_DIR, MP4_DIR, FRAMES_DIR, GIFS_DIR, TRANSCRIPTS_DIR, VISION_DIR, SKILLS_DIR, VAULT_DIR, HERMES_SKILLS_DIR]:
    ...
    log(f"Total skills: {len(list(HERMES_SKILLS_DIR.glob('davinci-reel-*')))}")
