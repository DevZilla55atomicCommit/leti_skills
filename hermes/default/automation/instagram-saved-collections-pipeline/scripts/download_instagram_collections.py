...
class CollectionDownloader:
        ...
        async with self.semaphore:
            ...
            for attempt in range(1, self.config.max_retries + 1):
                ...
                if attempt < self.config.max_retries:
                    wait = min(30 * (2 ** (attempt - 1)), 600)  # 30, 60, 120, 240, 480, max 600
    ...
    async def _execute_yt_dlp(self, url: str, attempt: int) -> bool:
        ...
        coll_output_dir = self.config.output_dir / self.collection_name
        coll_output_dir.mkdir(parents=True, exist_ok=True)

        output_template = str(coll_output_dir / f"{safe_id}.%(ext)s")
        ...
        try:
            ...
            if proc.returncode == 0:
                await self._verify_and_enhance_metadata(url, safe_id, coll_output_dir)
                return True
            ...
            self.logger.error(f"Timeout after {self.config.per_url_timeout}s: {url}")
            raise
        except Exception as e:
            self.logger.error(f"Exception: {e}")
            raise

    async def _verify_and_enhance_metadata(self, url: str, safe_id: str, coll_output_dir: Path):
        video_files = list(coll_output_dir.glob(f"{safe_id}.*"))
        ...
        json_file = coll_output_dir / f"{safe_id}.info.json"
        ...
        enhanced_path = coll_output_dir / f"{safe_id}.meta.json"
...
class MasterOrchestrator:
    ...
    def discover_collections(self) -> List[tuple]:
        """Return list of (collection_name, [urls]) for all _NEW.txt files."""
        collections = []
    ...
    def _write_final_report(self):
        ...
        report = {
            ...
            "config": {
                ...
                "per_url_timeout": self.config.per_url_timeout,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Full overnight run (3,504 URLs across 116 collections)
  python3 download_instagram_collections.py \\
    --urls-dir instagram_new_urls_by_collection \\
    --cookies cookies_www.instagram.com_2026-07-19.txt \\
    --output instagram_downloads \\
...
"""
    ...
    parser.add_argument("--max-concurrent", type=int, default=2,
                        help="Max concurrent downloads (default: 2, safe for Instagram)")
...
async def main():
    ...
    if not args.cookies.exists():
        ...
        print("Export Instagram cookies in Netscape format (e.g., via Get cookies.txt extension)", file=sys.stderr)
        sys.exit(1)
    ...
    if args.collection:
        all_collections = orchestrator.discover_collections()
        filtered = [(n, u) for n, u in all_collections if n == args.collection]
        if not filtered:
            print(f"Collection not found: {args.collection}", file=sys.stderr)
            print(f"Available: {[n for n, _ in all_collections]}", file=sys.stderr)
            sys.exit(1)
        orchestrator.discover_collections = lambda: filtered
