#!/usr/bin/env python3
"""
Complete Instagram Reels → DaVinci Resolve Knowledge Pipeline

This is the MAIN pipeline that processes Instagram Reels through all 6 stages:
1. Download via downreels.com (Playwright automation)
2. Extract frames (1fps), GIF (10fps full video), transcript (Whisper)
3. Vision analysis (3 frames per reel via vision_analyze)
4. Generate Hermes skills in DaVinci_Knowledge_Base/Hermes_Skills
5. Create Obsidian vault notes in DaVinci_Knowledge_Base/Instagram_Reels
6. Cleanup temp files

Usage:
    python3 auto_processor.py --urls-file urls.txt --batch-size 10
    python3 auto_processor.py --resume
    python3 auto_processor.py --dry-run

Critical Fixes Applied (v1.0.3):
- GIF copy to vault BEFORE cleanup (was being lost)
- VISION_REPORT_DIR defined explicitly
- NumPy/Python 3.14 workaround documented
- Vault path MUST be TamaZila Obsidian Vault exact path
"""
...
# ============================================================
# CONFIGURATION - MUST MATCH TAMAZILA VAULT EXACTLY
# ============================================================
...
# OUTPUT PATHS - ALL IN TAMAZILA VAULT
...
# CRITICAL: VISION_REPORT_DIR must be defined (was causing startup crash)
VISION_REPORT_DIR = VISION_DIR

# Rate limiting
VISION_RATE_LIMIT = 3.0  # seconds between vision calls (20 RPM max)
...
def extract_reel(reel_id: str, mp4_path: Path) -> Dict:
    ...
    # GIF: full video, 10fps, 480px
    run_cmd(f'ffmpeg -y -i "{mp4_path}" -vf "fps=10,scale=480:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" "{gif_path}"', timeout=120)
...
"""
    ...
    # CRITICAL FIX v1.0.3: Copy frames AND GIF to vault BEFORE cleanup
    ...
    # CRITICAL FIX v1.0.3: Copy GIF to vault
    gif_path = Path("~/instagram-davinci-pipeline/temp/gifs").expanduser() / f"{reel_id}.gif"
    if gif_path.exists():
        vault_gif = VAULT_DIR / collection / f"{reel_id}.gif"
        shutil.copy2(gif_path, vault_gif)
        log(f"Copied GIF to vault: {vault_gif.name} ({vault_gif.stat().st_size / 1024 / 1024:.1f} MB)")
...
async def main():
    ...
    parser.add_argument("--batch-size", type=int, default=DOWNLOAD_BATCH_SIZE)
